...
@pytest.mark.asyncio
async def test_store_and_count(adapter: SedimentAdapter):
...
@pytest.mark.asyncio
async def test_recall_returns_relevant(adapter: SedimentAdapter):
    ...
    # Sediment generates its own UUIDs, so we check content instead of IDs
    assert "Postgres" in results[0].content or "database" in results[0].content
    assert results[0].score is not None


@pytest.mark.asyncio
async def test_reset_clears_data(adapter: SedimentAdapter):
...
@pytest.mark.asyncio
async def test_store_with_scope(adapter: SedimentAdapter):
    await adapter.store(
        MemoryItem(
            id="m1",
