...
@pytest.mark.asyncio
async def test_store_and_count(adapter: ChromaDBAdapter):
...
@pytest.mark.asyncio
async def test_recall_returns_relevant(adapter: ChromaDBAdapter):
...
@pytest.mark.asyncio
async def test_reset_clears_data(adapter: ChromaDBAdapter):
...
@pytest.mark.asyncio
async def test_store_with_metadata(adapter: ChromaDBAdapter):
    await adapter.store(
        MemoryItem(
            id="m1",
